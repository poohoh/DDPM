...
def diffusion_defaults():
    defaults = dict(
        ...
        base_channels=64,
        channel_mults=(1, 2, 2),  # reduced # of downsampling <- input size is (28,28)
        num_res_blocks=2,
        time_emb_dim=128 * 4,
        norm="gn",
        dropout=0.1,
...
def get_diffusion_from_args(args):
    ...
    model = UNet(
        img_channels=1,  # MNIST has one channel
    ...
    diffusion = GaussianDiffusion(
        model, (28, 28),  # image size 28x28
        1,  # number of channels 1
        10,  # number of classes 10
        betas,

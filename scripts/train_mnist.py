...
from ddpm import script_utils_for_mnist
...
def main():
    ...
    try:
        diffusion = script_utils_for_mnist.get_diffusion_from_args(args).to(device)
        ...
        train_dataset = datasets.MNIST(
            root='/media/sda1/junha/DDPM/mnist_data',  # train dataset path
            train=True,
            download=True,
            transform=script_utils_for_mnist.get_transform(),
        )

        test_dataset = datasets.MNIST(
            root='/media/sda1/junha/DDPM/mnist_data',  # test dataset path
            train=False,
            download=True,
            transform=script_utils_for_mnist.get_transform(),
        )

        train_loader = script_utils_for_mnist.cycle(DataLoader(
            train_dataset,
...
def create_argparser():
    ...
    defaults = dict(
        ...
        log_dir="/media/sda1/junha/DDPM/mnist_ckpt",
        project_name="DDPM",
        run_name=run_name,
    ...
    defaults.update(script_utils_for_mnist.diffusion_defaults())

    parser = argparse.ArgumentParser()
    script_utils_for_mnist.add_dict_to_argparser(parser, defaults)
    return parser
